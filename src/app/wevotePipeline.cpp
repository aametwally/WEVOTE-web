...
struct WevoteParameters
{
    ...
    WevoteParameters()
        : threads{1}, penalty{2}, minNumAgreed{0}, score{0}, verbose{false} ,
          clark( false ), blastn( false ) , metaphlan( false ) , kraken( false ) , tipp( false )
    {}
};
        ...
        ParsingResults<WevoteParameters> &results)
{
    ...
    results.parameters.blastn =
            parser.isSet("blastn");
    results.parameters.clark =
            parser.isSet("clark");
    results.parameters.kraken =
            parser.isSet("kraken");
    results.parameters.metaphlan =
            parser.isSet("metaphlan");
    results.parameters.tipp =
            parser.isSet("tipp");
};
...
int main(int argc, char *argv[])
{
    ...
    if( param.blastn    ) algorithms.push_back("BLASTN");
    if( param.clark     ) algorithms.push_back("CLARK");
    ...
    std::vector< std::string > sequences = wevote::io::getFileLines( param.query );
    wevote::WevoteScriptHandler pipelineHandler;
    std::vector< wevote::ReadInfo > reads = pipelineHandler
            .execute( sequences , algorithms );
    ...
    /// Output.
    wevote::WevoteClassifier::writeResults( reads , algorithms , outputDetails );
